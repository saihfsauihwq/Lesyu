...
function Displayinfo($fileDb, $uid) {
    if (isset($uid) && $uid > 0) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $pubstat = isset($research['Publish_status']) ? $research['Publish_status']: "NO Publish Status.";
            $DOI = isset($research['DOI']) ? $research['DOI']: "NO DOI found.";

        } else {
            $title = $abstract = $date = $authors = $files = $strandID = $Description = $pubstat = $DOI = "Research paper not found";
        }
    } else {
        $title = $abstract = $date = $authors  = $files = $strandID = $Description = $pubstat = $DOI = "Invalid request";
    }

    return ['title' => $title, 'abstract' => $abstract, 'date' => $date, 'authors' => $authors, 'files' => $files, 'strands_id' => $strandID, 'Description' => $Description ,
            'pubstat' => $pubstat, 'DOI' => $DOI];
}
