...
include '/xampp/htdocs/WRRL/USER/CODES/REG-PAGE/Registration_process.php';
...
function verifyCode($email, $verification_code) {
    ...
    $sql = "SELECT * FROM user_accounts WHERE Liceo_Email = '$email' AND Verification_code = '$verification_code'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Verification successful, update user status or perform other actions as needed
        $sql = "UPDATE user_accounts SET Verified = 1 WHERE Liceo_Email = '$email'";
        $conn->query($sql);
