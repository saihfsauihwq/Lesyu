    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($password === $confirm_password) {
            ...
            if ($user_domain === $allowed_domain) {
                $sql = "INSERT INTO user_accounts (Profile,Firstname, Lastname, ID_number, Liceo_Email, Password, Strand, Grade_section, Gender, EStatus, Year_grad) 
                        VALUES 
                        ('$Profile','$Firstname', '$Lastname', '$ID_number', '$email', '$hashed_password', '$Strand', '$grade', '$Gender', '$status', '$year')";

                if ($conn->query($sql) === TRUE) {
                    // Generate and store verification code
                    $verification_code = generateVerificationCode();
                    $sql = "UPDATE user_accounts SET Verification_code = '$verification_code' WHERE Liceo_Email = '$email'";
                    $conn->query($sql);
