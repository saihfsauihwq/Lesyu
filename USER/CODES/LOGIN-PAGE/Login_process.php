...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (filter_var($login_input, FILTER_VALIDATE_EMAIL)) {
        $query = "SELECT * FROM user_accounts WHERE `Liceo_Email` = ?";
    } else {
    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $stored_hashed_password)) {
            $_SESSION["user_accounts"] = ["ID_number" => $row["ID_number"], "Liceo_Email" => $row["Liceo_Email"]];
            header("Location: http://localhost/WRRL/USER/Codes/HOME-PAGE/HOME-PAGE.php");
            exit();
        } else {
